...
// Now runnable — the pinned-Pyodide sqlite3 wheel is primed in the deno cache (see the
...
log(
  `independent state: #1.a=${x} (want 111), #2.a=${y} (want 222) -> ${
    x === 111 && y === 222 ? "ISOLATED OK" : "LEAK"
  }`,
);
log(
  `\nRESULT: dual sqlite3 ${
    a && b && x === 111 && y === 222
      ? "WORKS — Option A residual risk CLOSED"
      : "PROBLEM"
  }`,
);
