...
Deno.test("A′: credential is invisible to sandbox code and host auth is authoritative", async () => {
  ...
  py.globals.set("host_fetch", (m: string, u: string, h: string, _b: string): string => {
    const headers = JSON.parse(h);
    if (isModelRelayResponsesCall(m, u)) stripAndInjectAuth(headers, creds); // relay.ts A′ path
    sentHeaders = headers;
    return JSON.stringify({ output: [{ type: "message", role: "assistant", content: [{ type: "text", text: "ok" }] }] });
  });

  // The sandbox sees ONLY the credential-stripped request (relay.ts: request_json).
...
found = []
for k in ("api_key", "customer_token", "auth_type"):
    if k in req:
        found.append(k)
...
",".join(found)
`);
  assertEquals(leak, "", "no credential field or secret value may be reachable from the sandbox");
...
`);
  assertEquals(sentHeaders["Authorization"], "Bearer ct_SECRET", "host injects the real customer token");
  assert(
...
Deno.test("A′ legacy kill switch keeps the pre-split behavior (credential in the request)", () => {
  ...
  assert(legacyGlobal.includes("mr_sk_X"), "legacy path intentionally exposes the credential");
  const { sandboxRequest } = splitCredentials(request);
  assert(!JSON.stringify(sandboxRequest).includes("mr_sk_X"), "A′ path does not");
});
