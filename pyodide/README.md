...
| configured event descriptor (fd3 by convention) | Canonical Trace ABI v3 NDJSON only. |
...
- **Extract-fallback failure rate is unknown.** When `max_iterations` is
  ...
  `extract_error` (result field + the Trace ABI v3 `extract` failure event) instead of
