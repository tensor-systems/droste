...
// Pyodide package won't load in this env (pinned vs 314.0.2 cache drift; it fails
...
const EXPECT = JSON.stringify([{ id: 0, body: "hi" }, {
  id: 1,
  body: "there",
}]);

const rssMB = () =>
  Math.round((Deno.memoryUsage().rss / 1024 / 1024) * 10) / 10;
...
const mRepl = rssMB();
console.error(
  `REPL loaded in ${tReplLoad}ms; RSS ${mRepl} MB (+${
    (mRepl - m0).toFixed(1)
  })`,
);
...
const mBoth = rssMB();
console.error(
  `2nd interp loaded in ${tDbLoad}ms; RSS ${mBoth} MB (+${
    (mBoth - mRepl).toFixed(1)
  } for 2nd interp)`,
);
...
try {
  ...
  feasibleA = await repl.runPythonAsync(
    `query(${
      JSON.stringify(SQL)
    }) == [{"id":0,"body":"hi"},{"id":1,"body":"there"}]`,
  );
  try {
    await repl.runPythonAsync(`query("DELETE FROM messages")`);
  } catch {
    validatorA = true;
  }
} catch (e) {
  console.error(
    "OPTION A bridge FAILED:",
    e instanceof Error ? e.message.split("\n")[0] : String(e),
  );
}
console.error(
  `A feasible (cross-interp query correct): ${feasibleA}; validator enforced: ${validatorA}`,
);
...
const leakFS = await dbsvc.runPythonAsync(
  `__import__("os").path.exists("/tmp/repl_only.txt")`,
);
console.error(
  `isolation: globals leak=${leakGlobals} (want false), FS leak=${leakFS} (want false)`,
);

const latA: number[] = [];
for (let i = 0; i < CALLS; i++) {
  const s = now();
  await repl.runPythonAsync(`query(${JSON.stringify(SQL)})`);
  latA.push(now() - s);
}
console.error(
  `A latency/${CALLS}: median ${median(latA).toFixed(3)}ms  p95 ${
    p95(latA).toFixed(3)
  }ms`,
);
...
results.optionA = {
  feasible: feasibleA,
  validator_enforced: validatorA,
  globals_isolated: !leakGlobals,
  fs_isolated: !leakFS,
...
const MESSAGES = [{ id: 0, body: "hi" }, { id: 1, body: "there" }, {
  id: 2,
  body: "friend",
}];
const DANGER = [
...
const jsQueryB = async (sql: string): Promise<string> => {
  ...
  if (DANGER.some((k) => up.includes(k))) {
    throw new Error("SqlValidator: rejected non-read SQL");
  }
  return JSON.stringify(
    up.includes("ID < 3")
      ? MESSAGES.filter((m) => m.id < 3).slice(0, 2)
      : MESSAGES,
  );
};
repl.globals.set("_js_query", jsQueryB);
let feasibleB = false, validatorB = false;
feasibleB = await repl.runPythonAsync(
  `query(${
    JSON.stringify(SQL)
  }) == [{"id":0,"body":"hi"},{"id":1,"body":"there"}]`,
);
...
console.error(
  `B latency/${CALLS}: median ${median(latB).toFixed(3)}ms  p95 ${
    p95(latB).toFixed(3)
  }ms`,
);
...
results.optionB = {
  feasible: feasibleB,
  validator_enforced: validatorB,
  second_interp_rss_mb: 0,
...
results.env = {
  ...
  calls: CALLS,
  note:
    "pure-Python data layer; sqlite3 pkg unloadable in this env (cache drift), orthogonal to topology cost",
};
