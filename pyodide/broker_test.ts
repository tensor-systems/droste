...
import { authHeader, type Credentials, isModelRelayResponsesCall, splitCredentials, stripAndInjectAuth } from "./broker.ts";
...
Deno.test("isModelRelayResponsesCall: only POST https://api.modelrelay.ai/api/v1/responses qualifies", () => {
  ...
  assert(isModelRelayResponsesCall("POST", "https://api.modelrelay.ai/api/v1/responses"));
  assert(isModelRelayResponsesCall("post", "https://api.modelrelay.ai/api/v1/responses")); // case-insensitive method
  // Other ModelRelay endpoints must NOT get the credential (no general-purpose token).
  assert(!isModelRelayResponsesCall("POST", "https://api.modelrelay.ai/api/v1/account/card-verification/confirm"));
  assert(!isModelRelayResponsesCall("GET", "https://api.modelrelay.ai/api/v1/responses")); // wrong method
  assert(!isModelRelayResponsesCall("POST", "https://api.modelrelay.ai/api/v1/responses/extra")); // not exact path
  // Plaintext, other hosts, substring/lookalike, subdomain — all rejected.
  assert(!isModelRelayResponsesCall("POST", "http://api.modelrelay.ai/api/v1/responses"));
  assert(!isModelRelayResponsesCall("POST", "https://evil.com/?x=api.modelrelay.ai/api/v1/responses"));
  assert(!isModelRelayResponsesCall("POST", "https://api.modelrelay.ai.evil.com/api/v1/responses"));
  assert(!isModelRelayResponsesCall("POST", "https://staging.api.modelrelay.ai/api/v1/responses"));
...
Deno.test("splitCredentials removes every credential field from the sandbox request", () => {
  ...
  assert(!("customer_token" in sandboxRequest), "customer_token must not reach the sandbox");
  assert(!("auth_type" in sandboxRequest), "auth_type must not reach the sandbox");
  // Held host-side.
  assertEquals(creds, { authType: "customer_token", apiKey: "mr_sk_secret", customerToken: "ct_secret" });
});
...
Deno.test("splitCredentials tolerates a request with no credentials", () => {
  ...
  assertEquals(creds, { authType: "api_key", apiKey: undefined, customerToken: undefined });
});

Deno.test("authHeader: customer token takes precedence over api key", () => {
  const creds: Credentials = { authType: "customer_token", apiKey: "mr_sk_x", customerToken: "ct_y" };
...
Deno.test("stripAndInjectAuth: host auth overrides whatever the sandbox tried to set", () => {
  ...
  const out = stripAndInjectAuth(headers, { authType: "customer_token", customerToken: "ct_real" });
  assertEquals(out, { "Content-Type": "application/json", Authorization: "Bearer ct_real" });
...
Deno.test("stripAndInjectAuth: no credential means no auth header leaks through", () => {
  const headers: Record<string, string> = { "X-ModelRelay-Api-Key": "sandbox_guess" };
