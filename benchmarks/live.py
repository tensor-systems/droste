...
_OOLONG_SEMANTIC_GUIDANCE = (
    ...
    "non-overlapping chunks of that size, producing at most 20 chunks. Construct the chunks, "
    "schema, prompts, validator, and initial result exactly once so cell re-execution reuses "
    "the same objects:\n"
    "if 'oolong_result' not in globals():\n"
    "    oolong_chunk_size = max(1, math.ceil(len(records) / 20))\n"
    "    oolong_chunks = [\n"
    ...
    "            + 'Return one label for every record and preserve record order.\\n\\n'\n"
    "            + 'Records (one per line):\\n'\n"
    "            + '\\n'.join(oolong_chunk)\n"
    "            + '\\n\\nReturn only strict JSON whose labels string contains exactly one '\n"
    "            + 'ordered A/D/E/H/L/N character per input record, for example '\n"
    "            + '{\"labels\":\"ADEHLN\"}.'\n"
    "        )\n"
    ...
    "        if (\n"
    "            len(value['labels']) != len(oolong_chunks[index])\n"
    ...
    "\n"
    "    oolong_result = None\n"
    "This strict object schema requires labels to be a string and allows no additional "
    "properties. Each prompt includes its records and the rubric and requests only strict "
    "JSON whose labels string contains exactly one ordered A/D/E/H/L/N character per input "
    'record, for example {"labels":"ADEHLN"}. This compact string avoids unreliable JSON '
    "array element counting and stays well within the 2048-token subcall output bound. Do not "
    "switch back to a JSON array, multi-character codes, or a larger chunk count. The index "
    ...
    "validate_labels objects, for at most three total attempts:\n"
    "attempt = 0\n"
    "while (oolong_result is None or oolong_result['errors']) and attempt < 3:\n"
    "    oolong_result = llm_batch_json(oolong_prompts, oolong_schema, "
    "max_repair_attempts=1, validator=validate_labels)\n"
    "    attempt += 1\n"
    "result = oolong_result\n"
    "if result['errors']:\n"
    "    raise RuntimeError('classification failed')\n"
    "Never retry a subset or reconstruct any of those objects. With at most 20 chunks per "
    "full replay and up to 3 full replays, the worst case is roughly 3 x 20 = 60 calls "
    "(each replay already includes one internal max_repair_attempts round for items that "
    "fail), comfortably within the 150-call ceiling. Refuse any result with errors; never "
