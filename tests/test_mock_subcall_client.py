...
def test_mock_without_explicit_context_rebinds_for_sequential_runs() -> None:
    ...
    first = create_execution_context()
...
def test_mock_with_explicit_context_keeps_it_when_run_rlm_binds_another() -> None:
    explicit = create_execution_context()
    run_context = create_execution_context()
...
def test_mock_reports_batch_attempts_and_empty_outputs_are_unsuccessful() -> None:
    context = create_execution_context()
    mock = MockSubcallClient(context=context)

    assert mock.llm_batch(["one", "two"]) == ["", ""]
    assert mock.llm_query("another") == ""

    assert context.stats.calls_made == 3
    assert context.stats.successful_calls == 0


def test_mock_attempt_accounting_is_thread_safe() -> None:
    context = create_execution_context()
    mock = MockSubcallClient(context=context)
    ...
    assert context.stats.calls_made == 64
    assert context.stats.successful_calls == 0
