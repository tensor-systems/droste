...
def test_startup_reports_the_ready_gates_a_run_armed() -> None:
    """An ungated run must be distinguishable from a gated one in the trace.

    Enforcement is opt-in and defaults to off, so silence here used to mean
    both "the caller declined to gate" and "the caller forgot".
    """

    from droste.execution.trace import validate_event_body
